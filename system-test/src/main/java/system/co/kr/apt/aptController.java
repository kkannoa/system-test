...
//		ImageIO.write(image, "png", new File(dir.toString() + "/" + fileName + ".png"));
	...
	@RequestMapping("/aptNetwork") //lgj
	public ModelAndView aptNetwork(HttpSession session, ModelAndView mav) throws Exception {
		if (!SessionUtil.isNull(session, "ADMIN")) { // 세션확인
			apt.backController("aptNetwork", mav);	
		}else {mav.setViewName("redirect:../"); }//세션이 없을시 로그인으로 되돌아가기
		
		return mav;
	}
	
	
	@RequestMapping("/addAptNetwork") //lgj
	public ModelAndView addAptNetwork(HttpSession session, ModelAndView mav) throws Exception {
		if (!SessionUtil.isNull(session, "ADMIN")) {
			mav.setViewName("apt/addAptNetwork");
		}else {mav.setViewName("redirect:../"); }
		return mav;
	}
	
	@RequestMapping(value = "/addNetworkForm" , method = RequestMethod.POST) //lgj
	public ModelAndView addNetworkForm(HttpSession session, ModelAndView mav, 
			@ModelAttribute("getChangedInfo") networkVO networtVO) throws Exception {	
		if (!SessionUtil.isNull(session, "ADMIN")) {
	...
	@RequestMapping(value = "/delNetwork", method = RequestMethod.POST) //lgj
	public ModelAndView delNetwork(HttpSession session, ModelAndView mav,
			@RequestParam(value = "nSeqSiteH") String nSeqSite) 
			throws Exception {
...
//			System.out.println("percentList = " + percentList);
	...
	@RequestMapping(value = "/excelDownBadMeter", method = { RequestMethod.POST }) //lgj
...
//
	...
	public String lteIp(HttpSession session, Model model,
			@RequestParam(value = "jsonLte", defaultValue = "데이터없음") String jsonLte,
			@RequestParam(value = "snList", defaultValue = "데이터없음") String snList
			) throws Exception {
		if (!SessionUtil.isNull(session, "ADMIN")) {
			System.out.println("lteIp back ");
			System.out.println(jsonLte);
